...
`WECHAT_OAUTH_REDIRECT_URI` 必须是已登记网页授权域名下的完整 HTTPS 回调地址，例如 `https://promo.example.com/api/h5/auth/wechat/callback`。此处不要依赖请求头自动推导。
...
WECHATPAY_MCH_PRIVATE_KEY_PATH=/etc/betel-lottery/secrets/apiclient_key.pem
WECHATPAY_MCH_PUBLIC_KEY_ID=PUB_KEY_ID_...  # 微信支付平台公钥 ID，不是商户序列号
WECHATPAY_MCH_PUBLIC_KEY_PATH=/etc/betel-lottery/secrets/wechatpay/wechatpay_public_key.pem
...
启动校验只检查密钥**路径非空**，不验证文件真的可读；缺失的 PEM 会在首次签发转账单或验签回调时才报错。上线前用服务账号自查一次：
...
```bash
BETEL_ADMIN_PASSWORD='<随机强口令>' ./.venv/bin/python -m scripts.set_admin_password --username ops --password-from-env
```
...
将密钥放在受限目录并让 API 与 `wechat-pay-worker` 运行账号可读。不要把证书内容放入 `.env`、镜像或前端。
...
- `8000` 端口**不要**发布到公网或局域网；反向代理与 API 在同机时只监听 `127.0.0.1`。若确实存在直连公网的入口，必须 `TRUST_PROXY_HEADERS=false`。
...
0. 数据库初始化/升级必须在启动服务前显式完成。新空库可运行 `./.venv/bin/python -m scripts.init_db` 创建当前模型的表；它不会修改已有表。升级已有库前先用 `scripts/backup_mysql.sh` 备份，核对 `alembic/versions/`、模型与当前 revision，仅手动运行确认适用的迁移。服务启动不会自动迁移。
